...
async def help_command(message: types.Message):
    await message.answer(
        ...
        "💡 Уровни грехов:\n\n"
        "1 — потерянная/плохо сделанная задача, НЕ замеченная брокерами — не повлиявшая ни на что, но имевшая место\n\n"
        "2 — потерянная/плохо сделанная задача, замеченная брокерами — значительного вреда не оказала\n\n"
        "3 — при выполнении задачи ассистент стабильно не летит без пинка; серьёзная беспечность ассистента, повлекшая расход, которого можно было избежать / повлекшая существенный вред рабочему процессу"
    )
